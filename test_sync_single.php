...
if ($error) {
    ...
    if (isset($data['data']['allPeople']['data'])) {
        echo "\nSUCCESS! Found " . count($data['data']['allPeople']['data']) . " people.\n";
        foreach ($data['data']['allPeople']['data'] as $person) {
            echo "  - {$person['full_name']} ({$person['email']}) - LC: {$person['home_lc']['name']}, MC: {$person['home_mc']['name']}\n";
        }
    } else {
        echo "\nFAILED to parse people. Raw response structure:\n";
